#!/bin/bash
# Interactive version of apply-migration.sh
# This version allows you to enter the database password when prompted
# Usage: ./scripts/apply-migration-interactive.sh [migration-file]
...
if grep -q "project_id.*=.*\"$PROJECT_REF\"" supabase/config.toml 2>/dev/null; then
  ...
  echo "   (Press Enter to skip if you don't have it)"
...
echo "   You can find it in: Supabase Dashboard → Project Settings → Database"
echo "   (Press Enter to skip if you don't have it - some projects don't require it)"
echo ""
...
if supabase db push; then
  ...
  echo "Common issues:"
  echo "  - Database password: Get it from Supabase Dashboard → Project Settings → Database"
